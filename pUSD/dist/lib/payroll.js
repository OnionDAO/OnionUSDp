import { createTransaction, getMinimumBalanceForRentExemption, signTransactionMessageWithSigners } from "gill";
import { getCreateAccountInstruction, getAssociatedTokenAccountAddress, getCreateAssociatedTokenInstruction, getMintToInstruction, getTransferCheckedInstruction, TOKEN_PROGRAM_ADDRESS, TOKEN_2022_PROGRAM_ADDRESS } from "gill/programs";
import { PublicKey } from '@solana/web3.js';
import { TOKEN_CONSTANTS, PDA_SEEDS, PROGRAM_IDS, ERROR_MESSAGES } from '../config/constants';
...
export class PayrollManager {
     ...
     */
    derivePayrollEscrowPDA(batchId) {
        const [pda] = PublicKey.findProgramAddressSync([Buffer.from(PDA_SEEDS.PAYROLL_ESCROW), Buffer.from(batchId)], new PublicKey(PROGRAM_IDS.ONIONUSDP_PROGRAM));
        return pda;
    }
    /**
     * Schedule payroll batch
     */
    async schedulePayroll(authority, batchId, merkleRoot, total, releaseAt) {
        try {
            ...
            const authorityPusdATA = await getAssociatedTokenAccountAddress(this.pusdMint, authority.address, { programAddress: this.tokenProgram });
            const escrowPusdATA = await getAssociatedTokenAccountAddress(this.pusdMint, escrowPDA, { programAddress: this.tokenProgram });
     ...
     */
    async thawPayroll(authority, batchId, employees) {
        try {
            const { rpc, sendAndConfirmTransaction } = this.client;
            // Validate parameters
            ...
            for (const employee of employees) {
                const employeePusdATA = await getAssociatedTokenAccountAddress(this.pusdMint, employee.wallet, { programAddress: this.tokenProgram });
     ...
     */
    async getPayrollEscrow(batchId) {
        try {
     ...
     */
    validateMerkleProof(employee, merkleRoot, proof) {
