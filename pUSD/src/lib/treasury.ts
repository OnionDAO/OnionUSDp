 ...
 */
async function loadOrGenerateKeypair(keypairPath: string): Promise<Keypair> {
  try {
...
export class TreasuryService {
  public treasuryKeypair!: Keypair; // Make public for testing
  private corporateMultisig!: Keypair;
  private yieldWallet!: Keypair;
  public treasuryAuthority!: PublicKey;
  public corporateAuthority!: PublicKey;
  public yieldAuthority!: PublicKey;
  private treasuryUSDC!: PublicKey;
  private corporateUSDC!: PublicKey;
  private yieldUSDC!: PublicKey;
  private yieldPUSDC!: PublicKey;
  private revokableTransactions: Map<string, CorporateTransaction> = new Map();

  // Add public access to constants
  public readonly PUSDC_MINT = PUSDC_MINT;
  public readonly USDC_MINT = USDC_MINT;
   ...
   */
  public async generateEmployeeWallet(branchId: string): Promise<{ keypair: Keypair; account: TreasuryAccount }> {
   ...
   */
  public registerEmployeeWallet(publicKey: PublicKey, branchId: string): TreasuryAccount {
    return {
      publicKey,
      isEmployee: true,
      isCorporate: false,
   ...
   */
  public async buildCorporateDepositTx(
    user: PublicKey,
    uiAmount: number
   ...
   */
  public async buildCorporateWithdrawTx(
    user: PublicKey,
    uiAmount: number
  ): Promise<{ tx: Transaction; signers: Keypair[]; transactionId: string }> {
    ...
    if (corporateBal < usdcUnits) throw new Error('Corporate treasury undercollateralized');
    ...
    const tx = new Transaction({
      ...
      .add(
        createTransferInstruction(
          this.corporateUSDC,
          userUSDCATA,
          this.corporateAuthority,
          Number(netUsdcAmount),
          [],
          TOKEN_2022_PROGRAM_ID
        )
      )
    ...
    const corporateTx: CorporateTransaction = {
      signature: '',
      from: user,
   ...
   */
  public async buildEmployeeDepositTx(
    user: PublicKey,
    uiAmount: number
  ): Promise<{ tx: Transaction; signers: Keypair[] }> {
    if (uiAmount <= 0) throw new Error('Deposit amount must be positive');
   ...
   */
  public async buildEmployeeWithdrawTx(
    user: PublicKey,
    uiAmount: number
  ): Promise<{ tx: Transaction; signers: Keypair[] }> {
    if (uiAmount <= 0) throw new Error('Withdraw amount must be positive');
   ...
   */
  public async revokeCorporateTransaction(transactionId: string): Promise<boolean> {
   ...
   */
  public getRevokableTransactions(): CorporateTransaction[] {
   ...
   */
  public async getYieldBalances(): Promise<{ usdc: string; pusdc: string }> {
  ...
  public async checkPegInvariant(): Promise<boolean> {
    ...
    const treasuryReserves = BigInt(
      (await this.connection.getTokenAccountBalance(this.treasuryUSDC)).value.amount
    );
    const corporateReserves = BigInt(
      (await this.connection.getTokenAccountBalance(this.corporateUSDC)).value.amount
    );
   ...
   */
  public async redeemPUSDC(userKeypair: Keypair, uiAmount: number): Promise<string> {
   ...
   */
  public async executeCorporateToEmployeeConfidentialTransfer(
    corporateUser: PublicKey,
    employeeUser: PublicKey,
    uiAmount: number
  ): Promise<{ transactionId: string; result: string }> {
    if (uiAmount <= 0) throw new Error('Transfer amount must be positive');
    ...
    const result = confidential.transferConfidentialTokens(
      PUSDC_MINT.toBase58(),
      uiAmount,
      employeePUSDCATA.toBase58()
    );
   ...
   */
  private async configureAccountForConfidentialTransfers(accountPubkey: PublicKey): Promise<void> {
    try {
   ...
   */
  public async depositConfidentialTokens(
    ...
    try {
      ...
      const result = confidential.depositConfidentialTokens(
        PUSDC_MINT.toBase58(),
        uiAmount,
   ...
   */
  public async applyPendingBalance(accountPubkey: PublicKey): Promise<string> {
    try {
   ...
   */
  public async withdrawConfidentialTokens(
    accountPubkey: PublicKey,
    uiAmount: number
  ): Promise<string> {
    if (uiAmount <= 0) throw new Error('Withdraw amount must be positive');
    ...
    try {
      ...
      const result = confidential.withdrawConfidentialTokens(
        PUSDC_MINT.toBase58(),
        uiAmount,
        accountPubkey.toBase58()
      );
   ...
   */
  public async buildCorporateToEmployeeTransferTx(
    corporateUser: PublicKey,
    employeeUser: PublicKey,
    uiAmount: number
  ): Promise<{ tx: Transaction; signers: Keypair[]; transactionId: string }> {
    // This method now delegates to the CLI-based confidential transfer
    const { transactionId } = await this.executeCorporateToEmployeeConfidentialTransfer(
      corporateUser,
      employeeUser,
      uiAmount
    );
   ...
   * Legacy method for backward compatibility - now uses CLI
   */
  public async executeConfidentialTransfer(
    fromAccount: PublicKey,
    toAccount: PublicKey,
    uiAmount: number
  ): Promise<string> {
    // This method now delegates to the CLI-based confidential transfer
    const { result } = await this.executeCorporateToEmployeeConfidentialTransfer(
      fromAccount,
      toAccount,
      uiAmount
    );
